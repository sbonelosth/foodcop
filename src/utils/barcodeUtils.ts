...
export const validateBarcode = (
  ...
  for (const [range, country] of Object.entries(GS1_PREFIXES)) {
    ...
    if (end) {
      if (prefixNum >= parseInt(start, 10) && prefixNum <= parseInt(end, 10)) {
  ...
  return { isValid: countryCode !== "", countryCode, countryName };
};

// Fetches user's country code via IP geolocation.
